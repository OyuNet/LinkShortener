...
const tokenCleanCron = new CronJob(
    "0 0 * * * *",
    () => {
    cleanTokens();
    },
...
app.get("/register", rateLimit({
    windowMs: 60 * 1000,
    limit: 5,
    standardHeaders: "draft-7",
    legacyHeaders: false,
    message: "You have exceeded your registration quota per minute. Please wait"
}), async (req, res) => {
    ...
    await registerUser(username, password).then((status) => {
        status ? res.json({ "status": "ok", "username": username }) : res.json({ "status": "err", "error": status });
    }).catch((err) => {
        res.json({
            "status": "err",
            "error": err
